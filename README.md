...
Рассмотрим файл сборки данного проекта, чтобы понять как очень просто, быстро, удобно и на любой операционной системе подключить библиотеку Google Test.
    ...
    // Команда которая говорит линковать вашу тестовую программу статически, пока не вникаем, кому интересно, можете почитать в интернете про статическую и динамическую линковку
    // Если вкратце, то после этого нашу тестовую программу можно будет запускать на любом компьютере c поддерживаемой архитектурой процессора и ОС, где даже не установлена библиотека с++. 
